  ...
  <div>
    ...
    <div v-if="editing" class="card p-6 mb-8 border-l-4 border-terracotta-500">
      <h2 class="font-semibold text-stone-800 mb-5">
        {{ editingId ? t("admin.place_edit") : t("admin.place_add") }}
      </h2>
...
onMounted(async () => {
  const { data } = await (supabase as any)
    ...
    .eq("key", "map_password")
    .single();
  if (data?.value) newPassword.value = data.value;
});
...
const uploadFile = async (file: File): Promise<string | null> => {
  ...
  if (error) { saveError.value = error.message; return null; }
  const { data } = supabase.storage.from("riads").getPublicUrl(path);
